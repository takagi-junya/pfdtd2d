module tfsf_sin
    ...
    subroutine ejbnd_u(i0,i1,i2,i3)
        ...
        !$omp parallel do
        do i=i0,i1
            ez(i,j) = ez(i,j) + bezy(i,j)*hxinc_sinwave(i,j-1)
        enddo
        !$omp end parallel do
        
        !$omp parallel do
        do i=i2,i3
            ex(i,j) = ex(i,j) - bexy(i,j)*hzinc_sinwave(i,j-1)
        enddo
        !$omp end parallel do
    ...
    subroutine ejbnd_o(i0,i1,i2,i3)
        ...
        !$omp parallel do
        do i=i0,i1
            ez(i,j) = ez(i,j) - bezy(i,j)*hxinc_sinwave(i,j)
        enddo
        !$omp end parallel do
        
        !$omp parallel do
        do i=i2,i3
            ex(i,j) = ex(i,j) + bexy(i,j)*hzinc_sinwave(i,j)
        enddo
        !$omp end parallel do
    ...
    subroutine eibnd_l(j0,j1,j2,j3)
        ...
        !$omp parallel do
        do j=j0,j1
            ez(i,j) = ez(i,j) - bezx(i,j)*hyinc_sinwave(i-1,j)
        enddo
        !$omp end parallel do
        
        !$omp parallel do
        do j=j2,j3
            ey(i,j) = ey(i,j) + beyx(i,j)*hzinc_sinwave(i-1,j)
        enddo
        !$omp end parallel do
    ...
    subroutine eibnd_r(j0,j1,j2,j3)
        ...
        !$omp parallel do
        do j=j0,j1
            ez(i,j) = ez(i,j) + bezx(i,j)*hyinc_sinwave(i,j)
        enddo
        !$omp end parallel do
        
        !$omp parallel do
        do j=j2,j3
            ey(i,j) = ey(i,j) - beyx(i,j)*hzinc_sinwave(i,j)
        enddo
        !$omp end parallel do
    ...
    subroutine hjbnd_u(i0,i1,i2,i3)
        ...
        !$omp parallel do
        do i=i0,i1
            hx(i,j) = hx(i,j) + bmxy(i,j)*ezinc_sinwave(i,j+1)
        enddo
        !$omp end parallel do
        
        !$omp parallel do
        do i=i2,i3
            hz(i,j) = hz(i,j) - bmzy(i,j)*exinc_sinwave(i,j+1)
        enddo
        !$omp end parallel do
    ...
    subroutine hjbnd_o(i0,i1,i2,i3)
        ...
        !$omp parallel do
        do i=i0,i1
            hx(i,j) = hx(i,j) - bmxy(i,j)*ezinc_sinwave(i,j)
        enddo
        !$omp end parallel do
        
        !$omp parallel do
        do i=i2,i3
            hz(i,j) = hz(i,j) + bmzy(i,j)*exinc_sinwave(i,j)
        enddo
        !$omp end parallel do
    ...
    subroutine hibnd_l(j0,j1,j2,j3)
        ...
        !$omp parallel do
        do j=j0,j1
            hy(i,j) = hy(i,j) - bmyx(i,j)*ezinc_sinwave(i+1,j)
        enddo
        !$omp end parallel do
        
        !$omp parallel do
        do j=j2,j3
            hz(i,j) = hz(i,j) + bmzx(i,j)*eyinc_sinwave(i+1,j)
        enddo
        !$omp end parallel do
    ...
    subroutine hibnd_r(j0,j1,j2,j3)
        ...
        !$omp parallel do
        do j=j0,j1
            hy(i,j) = hy(i,j) + bmyx(i,j)*ezinc_sinwave(i,j)
        enddo
        !$omp end parallel do
        
        !$omp parallel do
        do j=j2,j3
            hz(i,j) = hz(i,j) - bmzx(i,j)*eyinc_sinwave(i,j)
        enddo
        !$omp end parallel do
    ...
    real(kind=8) function exinc_sinwave(i,j)
        use constants
        implicit none
        ...
        x = (i+0.5d0)*dx
        y = j*dy
        eth = cogam*einc_sinwave(x,y)
        eph = sigam*einc_sinwave(x,y)
        exinc_sinwave = vxthe*eth+vxphi*eph
    end function

    real(kind=8) function eyinc_sinwave(i,j)
        use constants
        implicit none
        integer,intent(in) :: i,j
        real(kind=8) :: x,y,eth,eph
        ...
        eth = cogam*einc_sinwave(x,y)
        eph = sigam*einc_sinwave(x,y)
        eyinc_sinwave =vythe*eth+vyphi*eph
    end function

    real(kind=8) function ezinc_sinwave(i,j)
        use constants
        implicit none
        integer,intent(in) :: i,j
        real(kind=8) :: x,y,eth
        ...
        y = j*dy
        eth = cogam*einc_sinwave(x,y)
        ezinc_sinwave = vzthe*eth
    end function

    real(kind=8) function hxinc_sinwave(i,j)
        use constants
        implicit none
        integer,intent(in) :: i,j
        real(kind=8) :: x,y,eth,eph
        x = i*dx
        y = (j+0.5d0)*dy
        eth = cogam*hinc_sinwave(x,y)
        eph = sigam*hinc_sinwave(x,y)
        hxinc_sinwave = uxthe*eth+uxphi*eph
    end function

    real(kind=8) function hyinc_sinwave(i,j)
        use constants
        implicit none
        integer,intent(in) :: i,j
        real(kind=8) :: x,y,eth,eph
        ...
        eth = cogam*hinc_sinwave(x,y)
        eph = sigam*hinc_sinwave(x,y)
        hyinc_sinwave = uythe*eth+uyphi*eph
    end function

    real(kind=8) function hzinc_sinwave(i,j)
        use constants
        implicit none
        integer,intent(in) :: i,j
        real(kind=8) :: x,y,eph
        ...
        y = (j+0.5d0)*dy
        eph = sigam*hinc_sinwave(x,y)
        hzinc_sinwave = uzphi*eph
    end function


    real(kind=8) function einc_sinwave(x,y)
        use constants
        implicit none
        real(kind=8) :: tau
        real(kind=8),intent(in) :: x,y
        tau = t+(r0x*x+r0y*y-dis)/vbk
        einc_sinwave = sinwave(tau)
    end function

    real(kind=8) function hinc_sinwave(x,y)
        use constants
        implicit none
        real(kind=8) :: tau
        real(kind=8),intent(in) :: x,y
        tau = t+(r0x*x+r0y*y-dis)/vbk
        hinc_sinwave = sinwave(tau)
    end function

    real(kind=8) function sinwave(tau)
        use constants
        implicit none
        ...
        sinwave = amp*w*sin(omega*tt)
    end function sinwave
